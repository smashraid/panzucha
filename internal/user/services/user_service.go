...
import (
	...
	sd "panzucha/internal/shared/domain"
...
func (s *userService) Register(ctx context.Context, email, name, password string) (*domain.User, error) {
	...
	if err != nil && !errors.Is(err, sd.ErrNotFound) {
		// Real DB error — propagate, don't swallow.
		return nil, err
	}
	if existing != nil {
		return nil, sd.ErrConflict
	}
	...
	user := &domain.User{
		...
		Role:         "user",
		Audit:        sd.Audit{CreatedBy: email, UpdatedBy: email},
	}
...
func (s *userService) Login(ctx context.Context, email, password string) (string, error) {
	...
	if err != nil {
		...
		if errors.Is(err, sd.ErrNotFound) {
			return "", sd.ErrUnauthorized
		}
		return "", err
	}
...
func (s *userService) Update(ctx context.Context, id, email, name, updatedBy string) (*domain.User, error) {
	user := &domain.User{
		...
		Email: email,
		Name:  name,
		Audit: sd.Audit{UpdatedBy: updatedBy},
	}
