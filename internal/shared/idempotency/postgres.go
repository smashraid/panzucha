...
var _ IdempotencyKeyRepository = (*PostgresIdempotencyKeyRepository)(nil)
...
func (r *PostgresIdempotencyKeyRepository) Create(ctx context.Context, key *IdempotencyKey) error {
	const q = `
	...
	tag, err := r.pool.Exec(ctx, q,
		key.Key, key.ResourceType, IdempotencyStatusProcessing, key.ExpiresAt,
	)
	if err != nil {
		return err
	}
...
func (r *PostgresIdempotencyKeyRepository) FindByKey(ctx context.Context, key string) (*IdempotencyKey, error) {
	const q = `
